...
BF <- function(postsamps, over.null = TRUE){
  ...
  if(over.null){
    BF01 <- prior/posterior # if > 10 support for H0
    return(BF01)
  }
  else if(!over.null){
    BF10 <- posterior/prior # if > 10 support for H1
    return(BF10)
  }
}

# Calculate Bayes Factor for chance comparisons
BF.chance <- function(postsamps, over.chance = TRUE){
  library(polspline)
  fit_posterior <- logspline(postsamps) 
  posterior <- dlogspline(0, fit_posterior) # Height of the posterior at 0 
  prior <- dnorm(0, .5, 1) # Height of the prior at 0
