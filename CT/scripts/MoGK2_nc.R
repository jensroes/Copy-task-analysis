...
#d %>% select(subj, component) %>% count() %>% filter(freq < 3)
...
#---- 
# Load model
mog <- stan_model(file = "stanin/MoGK2_nc.stan")

# Check model
#m <- sampling(mog, chain = 1, iter = 1, data = dat) 

# Parameters to omit in output
omit <- c("mu1", "mu2", "sigma_e_comp", "sigmap_e_comp", "log_theta", "log_theta_comp", "beta_raw", "theta_tilde")#"L_u", "z_u",
